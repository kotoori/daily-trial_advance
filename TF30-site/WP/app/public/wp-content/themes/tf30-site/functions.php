...
function custom_pagination($current_page, $total_pages) {
  ...
  // 「一番前に戻る」ボタン
  if ($current_page > 1) {
      $output .= '<a href="' . get_pagenum_link(1) . '"><<</a>';
  }

  // 「前のページへ」ボタン
  if ($current_page > 1) {
      $output .= '<a href="' . get_pagenum_link($current_page - 1) . '"><</a>';
  }
  ...
  for ($i = 1; $i <= $total_pages; $i++) {
      ...
      // 3ページ以降を「...」で省略
      if ($i == 3 && $current_page < $total_pages - 1) {
          $output .= '<span>...</span>';
          $i = $total_pages - 1; // 最後のページへジャンプ
      }
  }
  ...
  // 「一番後に進む」ボタン
  if ($current_page < $total_pages) {
      $output .= '<a href="' . get_pagenum_link($total_pages) . '">>></a>';
  }
...
function custom_pagination_with_first_last($args, $current_page, $total_pages) {
  ...
  if ($total_pages > 3) {
      if($current_page > 1){
        $first_page_link = '<a href="' . get_pagenum_link(1) . '"><<</a> ';
      }
      if($current_page < $total_pages){
        $last_page_link = ' <a href="' . get_pagenum_link($total_pages) . '">>></a>';
      }
  }

  return $first_page_link . $links . $last_page_link;
}
